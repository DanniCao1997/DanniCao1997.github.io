...
If you are going to use R, the most important part is to apply function in R or writing R function yourself, which will save you time effectively.
...
R function can also assign the default value by stating the parameter equal to some value directly in the parameter setting bracket. Note that the parameter with default will also be assigned value as ordered. The assignment will not skip the parameter with default.
...
We will usually use a function in the format as "Function" "(Parameter)". However, there do exist some function put in the middle of two parameter. Here, you need to add the "%" before and after your infix function. Note that you need to use 
